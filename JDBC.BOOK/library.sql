...
INSERT INTO customer VALUES (2,'mslove','김민수',30,'인천광역시 남동구 정각로 29(구월동) 23-4 205호','F','12/03/11');
...
INSERT INTO library VALUES(2,3,'mslove','18/04/02','18/04/04');
...
SELECT LEASE_NO, L.USER_ID, USER_NAME, BOOK_NAME
FROM BOOK B, CUSTOMER C, LIBRARY L
WHERE C.USER_ID = L.USER_ID
AND B.BOOK_NO = L.BOOK_NO
AND BOOK_NAME LIKE '열혈강호';

COMMIT;
ROLLBACK;

DELETE FROM LIBRARY WHERE LEASE_NO LIKE '5';

INSERT INTO LIBRARY
VALUES(5, (SELECT BOOK_NO
FROM BOOK
WHERE BOOK_NAME LIKE '열혈강호'), 'user11', DEFAULT, SYSDATE + 2);
